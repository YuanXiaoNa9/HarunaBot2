...
impl FnHandler for GCMDeleteName {
    ...
    async fn process(&self, msg: &Msg) -> Result<(), Error> {
        ...
        if res.len() == 0 {
            ...
            sqlx::query!("delete from gc_name where gc_id = $1 and name = $2 and gid = $3",data.gc_id,data.name,msg.group_id).execute(&mut *tx).await?;
            tx.commit().await?;
            let mut  rep = SendMsg::new().await;
            rep.join_reply(msg.message_id).await;
            rep.join_text(format!("已删除机厅id：{}的名字：{}",data.gc_id,data.name)).await;
    ...
    async fn help(&self, _: &str) -> String {
        "删除名字\n注：\n当仅剩一个名字时无法使用\n使用方法：\n/机厅管理 删除名字 <id(可选)> <name>".to_string()
    }
