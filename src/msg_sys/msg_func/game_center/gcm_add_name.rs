...
impl FnHandler for GCMAddName {
    ...
    async fn process(&self, msg: &Msg) -> Result<(), Error> {
        ...
        if res.len() == 0 {
            ...
            for res in res {
                rep.push_str(format!("\nid: {}\nname: {}\n备注: {}", res.gc_id, res.name.unwrap(), res.description).as_str());
            }
            return Err(anyhow!(rep))
        }
        let old_names = res[0].name.clone().unwrap();
        let old_name_split = old_names.split(" ");
        for old_name in old_name_split {
            if old_name.contains(vec_splits[3]){
                return Err(anyhow!("名字出现重复，不要为一个机厅添加相同的名字"))
            }
        }
        sqlx::query!("insert into gc_name (gc_id,gid,name)values ($1,$2,$3)",res[0].gc_id,msg.group_id,vec_splits[3]).execute(&mut *tx).await?;
        tx.commit().await?;
        let mut rep = SendMsg::new().await;
        rep.join_text(format!("添加名字成功\nid: {}\nname: {} {}\n备注: {}",res[0].gc_id,old_names,name,res[0].description)).await;
