...
use tracing::debug;
use crate::msg_sys::func_mod::gc_name::{GcData, GCNAME};
...
impl FnHandler for GCMReDescription {
    ...
    async fn process(&self, msg: &Msg) -> Result<(), Error> {
        ...
        let gc_id:i64 = if vec_msg[2].parse::<i64>().is_err() {
            let gc_id = match GCNAME.map.read().await.get(format!("{}|{}",vec_msg[2],msg.group_id).as_str()){
        ...
        let res = sqlx::query_as!(Data,"select admin_gid,description from gamecenterdata where gc_id = $1", gc_id).fetch_all(&mut *tx).await?;
        if res.len() < 1 {
        ...
        sqlx::query!("update gamecenterdata set description = $1 where gc_id = $2",vec_msg[3],gc_id).execute(&mut *tx).await?;
        tx.commit().await?;
        let mut rep = SendMsg::new().await;
        rep.join_reply(msg.message_id).await;
        rep.join_text(format!("修改成功\nid: {}\n原备注: {}\n备注: {}",gc_id,res[0].description, vec_msg[3])).await;
    ...
    async fn help(&self, _: &str) -> String {
        "修改机厅备注(建议备注地名)\n使用方法：\n/机厅管理 修改备注 <机厅id/name> <备注内容>".to_string()
    }
