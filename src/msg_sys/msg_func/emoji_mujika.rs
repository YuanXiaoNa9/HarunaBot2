...
static TTF: LazyLock<Result<FontVec,bool>> = LazyLock::new(ttf);
...
fn ttf() ->Result<FontVec,bool> {
    match FontVec::try_from_vec(include_bytes!("/home/q/rust/HarunaBot2/ttf/siyuan.ttf").to_vec()){
        Ok(ttf) => {Ok(ttf)},
        Err(_) => {Err(false)}
    }
}
...
impl MsgHandler for EmoMjk {
    ...
    async fn process(&self, msg: Arc<Msg>) {
        ...
        std::fs::remove_file(format!("{}/{}.png",PATH.as_str(), file_name).to_string())
            .unwrap();
    }

    async fn init(&mut self) {
        self.ttf = match FontVec::try_from_vec(include_bytes!("/home/q/rust/HarunaBot2/ttf/siyuan.ttf").to_vec()) {
            Ok(ttf) => {OnceLock::from(ttf)},
            Err(_) => {self.status = false;return}
        };
    }
