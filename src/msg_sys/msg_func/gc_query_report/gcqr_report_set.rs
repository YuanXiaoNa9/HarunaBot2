...
impl FnHandler for GcqrSet {
    ...
    async fn process(&self, msg: &Msg) -> Result<(), Error> {
        let gc_name = msg.raw_message.trim_end_matches(|c:char|c.is_ascii_digit());
        ...
        let gc_id = GCNAME.map.read().await.get(format!("{}{}",gc_name,msg.group_id).as_str()).ok_or_else(|| Error::msg("no gc name found"))?.gc_id;
